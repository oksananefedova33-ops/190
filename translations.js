(function(){
    ...
    function updateLanguagesList() {
        ...
        // Сбрасываем текущие языки
        currentLanguages = [];
        
        // 1. Получаем языки из нативного элемента langbadge (если есть)
        ...
        // 2. Получаем языки из HTML-элементов (вставленных через HTML Preview)
        const imageElements = document.querySelectorAll('.el[data-type="image"]');
        imageElements.forEach(el => {
            const htmlContent = el.innerHTML;
            
            // Ищем lang-option элементы в HTML-контенте
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = htmlContent;
            
            const langOptions = tempDiv.querySelectorAll('.lang-option[data-lang]');
        ...
        currentLanguages = [...new Set(currentLanguages)];
        ...
        if (currentLanguages.length > 0) {
            ...
            container.innerHTML = '<div style="color:#6b7280">Нет выбранных языков. Используйте кнопку "Языки" или вставьте HTML-код языкового переключателя.</div>';
        }
    }
